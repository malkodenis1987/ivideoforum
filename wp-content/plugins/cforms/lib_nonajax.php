...
//
	...
	$subID = ( substr(get_option('cforms'.$no.'_tellafriend'),0,1)=='2' && !$send2author )?'noid':write_tracking_record($no,$field_email);
	...
	if ( isset($_FILES['cf_uploadfile'.$no]) ) {
		foreach( $_FILES['cf_uploadfile'.$no][tmp_name] as $tmpfile ) {
            ...
            if ( is_uploaded_file($tmpfile) )
            	move_uploaded_file($tmpfile,$fileuploaddir.'/'.$subID.'-'.$file['name'][$filefield]);
        	$filefield++;
		}	 
	}
	...
	if ( substr(get_option('cforms'.$no.'_tellafriend'),0,1)=='2' && $track['send2author']=='1'){
	...
	$frommail = check_cust_vars(stripslashes(get_option('cforms'.$no.'_fromemail')),$track,$no);
	if ( $frommail=='' )
	...
	if( $sentadmin==1 ) {
		    ...
		    if ( (get_option('cforms'.$no.'_confirm')=='1' && $field_email<>'') || ($ccme&&$trackf[$ccme]<>'-') )  // not if no email & already CC'ed
		    {
						...
						if ( $taf_youremail && $taf_friendsemail && substr(get_option('cforms'.$no.'_tellafriend'),0,1)=='1' ) //TAF: add CC
