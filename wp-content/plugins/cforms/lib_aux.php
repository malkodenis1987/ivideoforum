...
function write_tracking_record($no,$field_email){
		...
		if ( get_option('cforms_database') == '1'  ) {
			...
			$page = (substr(get_option('cforms'.$no.'_tellafriend'),0,1)=='2')?$_POST['cforms_pl'.$no]:get_current_page(); // WP comment fix
...
function check_default_vars($m,$no) {
		...
		if ( substr(get_option('cforms'.$no.'_tellafriend'),0,1)=='2' ) // WP comment fix
			$page = $permalink;
