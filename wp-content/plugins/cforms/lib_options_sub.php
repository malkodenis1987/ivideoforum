<?php
	...
	for($i = 1; $i <= $field_count; $i++) {
	  ...
	  if ($_REQUEST['field_' . $i . '_name']<>''){    // Newly "AddField" does not exist yet!
				...
				if( in_array($type, array('cauthor','email','url','comment','send2author')) && !(substr(get_option('cforms'.$no.'_tellafriend'),0,1)=='2') ){
				...
				if( in_array($type, array('yourname','youremail','friendsname','friendsemail')) && !(substr(get_option('cforms'.$no.'_tellafriend'),0,1)=='1') ){
...
*/
	...
	update_option('cforms'.$no.'_fname',    preg_replace(array('/\\\+/','/\//','/"/'), array('\\','-','\''), $_REQUEST['cforms_fname']) );
	...
	if ( isset($_REQUEST['cforms_commentrep']) )
		update_option('cforms'.$no.'_tellafriend',	($_REQUEST['cforms_commentrep']?'2':'0').($_REQUEST['cforms_commentXnote']?'1':'0') );
