...
*) feature: added default states for check boxes, radio boxes, drop downs etc.
...
*) other: some adjustments to the file upload handling routine
...
function cforms($args = '',$no = '') {
	...
	if ( substr(get_option('cforms'.$no.'_tellafriend'),0,1)=='2' && $send2author ) ### called from lib_WPcomments ?
		return $all_valid;
	...
	### 
	$success=false;
	if ( isset($_GET['cfemail']) && substr(get_option('cforms'.$no.'_tellafriend'),0,1)=='2' ){ ###  fix for WP Comment (loading after redirect)
	...
	else if( substr(get_option('cforms'.$no.'_tellafriend'),0,1)=='2' )
	...
	for($i = 1; $i <= $field_count; $i++) {
		...
		if ( $fielderr <> '')	{
		    ...
		    switch ( $field_type ) {
			    ...
			    default:				    
					...
					$custom_error .= (get_option('cforms'.$no.'_customnames')=='1')?cf_sanitize_ids($input_name[1]):'cf'.$no.'_field_'.$i;
		...
		if( !isset($_REQUEST['sendbutton'.$no]) && isset($_REQUEST[$input_name]) )
	...
	if ( $captcha ){
		...
		$content .= $nttt .'<input type="hidden" name="cforms_cap'.$no.'" id="cforms_cap'.$no.'" value="' . md5($tu) . '" alt="'.$captcha['i'].'"/>';
	}
	...
	if ( get_option('cforms'.$no.'_tellafriend')>0 ){
		if ( substr(get_option('cforms'.$no.'_tellafriend'),0,1)==2 ) 
			$nono = ''; else $nono = $no;
...
function cforms_insert( $content ) {
	...
	if ( ($a=strpos($content,'<!--cforms'))!==false ) {  ### only if form tag is present!
		...
		for ($i=0;$i<$forms;$i++) {
			$no = ($i==0)?'':($i+1);
			$fns[sanitize_title_with_dashes(get_option('cforms'.$no.'_fname'))] = $i+1;
		}
